...
class RoomDAO(BaseDAO):
    ...
    async def find_all(cls, hotel_id: int, date_from: date, date_to: date):
        ...
        SELECT
            rooms.*,
            (price * (date_to - date_from)) AS total_cost,
            (quantity - COALESCE(rooms_booked, 0)) AS rooms_left
        FROM rooms
        ...
        get_rooms = (
            select(
                ...
                (
                    Rooms.quantity - func.coalesce(booked_rooms.c.rooms_booked, 0)  # noqa: E501
                ).label("rooms_left"),
            )
            .join(
                booked_rooms,
